...
@section('sub_hero','Danh sách vật phẩm')
                ...
                <div class="col-12 vip-bordered">
                    ...
                    <div class="tab-content p-3">
                        <div class="tab-pane fade show active" id="home-index" role="tabpanel" aria-labelledby="index-tab">
                                ...
                                <tbody>
                                    @foreach($items as $key => $item)
                                        <tr>
                                            <td>
                                                <div class="checkbox">
                                                    <label class="ui-check ui-check-md">
                                                        <input value="{{ $item->id }}" type="checkbox" v-model="selected"> 
                                            ...
                                            <td @click="showInforItem({{ json_encode($item) }},0)" style=""><div style="" class="pixel {{ $item->class_tag }}"></div></td>
                                            <td style=""><small class="text-muted">{{ $item->users->count() }}</small></td>
                                            <td style=""><small class="text-{{ $item->price_type == 0 ? 'gold' : 'info' }}">{{ number_format($item->price) }} {{ $item->price_type == 0 ? 'Vàng' : 'KC' }}</small></td>
                                            <td style=""><small class="text-{{ $item->status == 1 ? 'success' : 'danger' }}">{{ $item->status == 1 ? 'Hiện' : 'Ẩn' }}</small></td>
                                            <td class="flex" style="">
                                                <div class="dropdown mb-2"><button class="btn btn-white dropdown-toggle" data-toggle="dropdown"
                                                    ...
                                                    <div class="dropdown-menu bg-dark" role="menu" x-placement="bottom-start"
                                                        ...
                                                        <a href="{{ Route('admin.items.edit',['id' => $item->id]) }}" class="dropdown-item">Xem & Sửa</a>
                                                        <a onclick="return confirm('Sao chép ?')" href="{{ Route('admin.items.replicate',['id' => $item->id]) }}" class="dropdown-item">Sao Chép</a>
                                                        <a onclick="return confirm('Xóa ?')" href="{{ Route('admin.items.delete',['id' => $item->id]) }}" class="dropdown-item">Xóa</a>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
                            ...
                            <form id="form" class="form" action="{{ Route('admin.items.delete-multi') }}" method="POST">
                                @csrf
