                                            ...
                                            <div style="background-color: #554839;padding:0px" class="tab-content" id="v-pills-tabContent">
                                                <div class="tab-pane fade show active" role="tabpanel">
                                                    <div class="b-b">
                                                        <div class="nav-active-border b-primary bottom">
                                                            <ul class="nav" id="myTab" role="tablist">
                                                                <li v-for="(menu, index) in menuShop" :key="index" data-title="tooltip" :title="menu.name" class="nav-item">
                                                                    <a class="nav-link" :class="{'active': index == 0}" :id="`total-user-equipment-${$parent.strSlug(menu.name)}-tab`" data-toggle="tab" :href="`#total-user-equipment-${$parent.strSlug(menu.name)}-tab-control`" role="tab" aria-controls="" aria-selected="true">
                                                                        {{ menu.name }}
                                                                    </a>
                                                                </li>
                                                            </ul>
                                                    ...
                                                    <div class="tab-content">
                                                        <div v-for="(menu, index) in menuShop" :key="index" class="tab-pane fade" :class="{'show active': index == 0}" :id="`total-user-equipment-${$parent.strSlug(menu.name)}-tab-control`" role="tabpanel" aria-labelledby="total-user-equipment-tab">
                                                            <div class="row p-2">
                                                                <div class="p-1 col-12">
                                                                    <div style="background: rgb(199, 188, 152)" class="item-preview">
                                                                        <div style="max-height:400px;overflow:auto" class="row px-4 py-2">
                                                                            <div v-for="(item, i) in $parent.profileInventory[$parent.strSlug(menu.name)]" :key="i" @click="$parent.showGearsDescription(item, 1)" style="position:relative; background: rgb(82, 74, 60);margin: 10px; border-radius:3px" class="p-0 col-auto">
                                                                                <div style="margin: 7px;border-radius; 3px;background-color: rgb(61, 56, 47);display:block;object-fit:contain;" :class="`pixel ${item.shop_tag}`"></div>
